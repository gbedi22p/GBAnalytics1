...
BEGIN
	...
	--released new enhanced with additional resellers so more people used it
	--use these variables if want to execute just this section of code
	--DECLARE @MAX_STAGE_1_ROWS_CNT INT = 300000
	--DECLARE @MAX_STAGE_2_ROWS_CNT INT = @MAX_STAGE_1_ROWS_CNT / 2
	--DECLARE @MIN_TOTAL_ORDERS_TO_ADD INT = @MAX_STAGE_2_ROWS_CNT / 2
	--DECLARE @MIN_PCT_USERS_CNT FLOAT = 0.03
	--DECLARE @MAX_PCT_USERS_CNT FLOAT = 0.05
	--DECLARE @START_ORDER_DATE DATE = '01/01/2021'
	--DECLARE @END_ORDER_DATE DATE = '08/31/2022'
	SET @MIN_TOTAL_ORDERS_TO_ADD INT = @MAX_STAGE_2_ROWS_CNT / 2
	SET @MIN_PCT_USERS_CNT FLOAT = 0.03
	SET @MAX_PCT_USERS_CNT FLOAT = 0.05
	SET @START_ORDER_DATE DATE = '01/01/2021'
	SET @END_ORDER_DATE DATE = '08/31/2025'
	EXEC dbo.GENERATE_MORE_ITEMS_IN_ORDERS_AND_ORDERS @MIN_PCT_USERS_CNT, @MAX_PCT_USERS_CNT, 
													  @MIN_TOTAL_ORDERS_TO_ADD, @START_ORDER_DATE, @END_ORDER_DATE
	--overall agg analysis
	SELECT COUNT(*), created_at FROM RESELLERS_2ND_HAND_STUFF_ORDERS

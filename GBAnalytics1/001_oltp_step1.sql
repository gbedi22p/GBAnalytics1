...
INSERT INTO ##TEMP_URL_TABLE(id, url)
	SELECT ROW_NUMBER() OVER (Order by subq.val), CONCAT('https://www.', subq.val, '/', subq2.val, '.com')
	FROM ##TEMP_WORDS_TABLE subq
	CROSS APPLY 
...
DROP TABLE IF EXISTS ##TEMP_ROWSET_ALL_INTS
CREATE TABLE ##TEMP_ROWSET_ALL_INTS
(
	id INT,
	val INT
)
